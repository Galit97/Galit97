## Hi, I’m Galit 👋

I’m a freelance **Full-Stack Web Developer** focused on **React / Next.js**.  
I build products end-to-end, from **product thinking and UX messaging**, through **design handoff**, to **clean, production-ready development**.

- Freelance developer
- 1 year startup experience (built features for a **CRM system**)
- Focus: UX-driven UI, clean code, and product-first execution



## Tech Stack

**Frontend:** React, Next.js, TypeScript, JavaScript, HTML, CSS/SCSS  
**Backend:** Node.js, Express.js, REST APIs  
**Databases:** MongoDB, SQL  
**UI:** Ant Design  
**WordPress:** WordPress, Elementor, WooCommerce  
**Dev & Deploy:** Git, GitHub/GitLab, VS Code, Cursor, Render, Netlify, Hostinger  
**Design & UX:** Figma, Canva  
**Automation & Tools:** Brevo, Airtable, Monday.com, ManyChat, Excel/Google Sheets, Google Drive  
**AI Tools:** ChatGPT, Copilot, Gemini



## Featured Projects

- **Wedi App** — https://github.com/Galit97/wedi-app  
- **Girls Aloud Jump Game** — https://github.com/Galit97/Icy-Tower
- **Easy Choice** — https://github.com/Galit97/Easy-choice  
- **More on my GitHub** — https://github.com/Galit97



## Contact

📩 Email: online.galit@gmail.com  
🌐 Website: https://galit-online.com  
💼 LinkedIn: https://linkedin.com/in/galit-lioutrin-a1b592345
